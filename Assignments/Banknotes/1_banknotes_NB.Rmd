...
```{python}
classifiers = [None] * 3
for i, (train, test, features_names) in enumerate([(train, test, features_names), (train_2, test_2, features_2), (train_3, test_3, features_3)]):
...
auc_scores = []
for i, (test_set, features_names) in enumerate([(test, features_names), (test_2, features_2), (test_3, features_3)]):
    print(i)
    ...
    plt.plot(fpr, tpr, label=f"{len(features_names)} features (AUC={auc:.2f})", linewidth=8/(i+1))
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
